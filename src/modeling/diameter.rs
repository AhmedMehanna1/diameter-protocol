...
use crate::modeling::avp::avp::{Avp, AvpHeader};
...
impl DiameterMessage {
    ...
    pub fn decode_from<R: Read>(reader: &mut R) -> DiameterResult<DiameterHeader> {
        let mut b = [0u8; 20];
        ...
        let avp_header = AvpHeader::decode_from(reader)?;
        println!("message length: {} avp length: {}", message_length, avp_header.length);

        // while message_length != 0 {
        //     let avp_header = AvpHeader::decode_from(reader)?;
        //     println!("message length: {} avp length: {}", message_length, avp_header.length);
        //     println!("avp header: {:?}", avp_header);
        //     message_length -= avp_header.length;
        // }
